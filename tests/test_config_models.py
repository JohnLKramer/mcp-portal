...
@pytest.mark.parametrize("base_url", ["api.example.com", "/v1", "ftp://api.example.com", "https://"])
...
def test_a_parameter_style_other_than_form_is_rejected():
    binding = MINIMAL["operations"][0]["binding"] | {"parameters": [{"arg": "ids", "in": "query", "style": "spaceDelimited"}]}
...
def test_introspect_unsafe_with_acknowledgement_is_accepted():
    cfg = McpPortalConfig.model_validate(MINIMAL | {"mode": "introspect-unsafe", "acknowledge_unsafe": True})
...
def test_upstream_may_rely_on_introspection_instead_of_base_url():
    payload = MINIMAL | {
        ...
        "upstreams": {"billing": {"introspection": {"openapi": {"url": "https://api.example.com/openapi.json"}}}},
    }
    cfg = McpPortalConfig.model_validate(payload)
    assert cfg.upstreams["billing"].base_url is None
    assert cfg.upstreams["billing"].introspection.openapi.url == ("https://api.example.com/openapi.json")


def test_configured_mode_requires_base_url_even_when_introspection_is_present():
    payload = MINIMAL | {"upstreams": {"billing": {"introspection": {"openapi": {"url": "https://api.example.com/openapi.json"}}}}}
    with pytest.raises(ValidationError) as exc:
        McpPortalConfig.model_validate(payload)
...
def test_openapi_introspection_requires_exactly_one_of_url_or_file():
    ...
    with pytest.raises(ValidationError):
        McpPortalConfig.model_validate(with_openapi({"url": "https://api.example.com/openapi.json", "file": "./openapi.json"}))
...
def test_local_principal_accepts_a_list_of_raw_authorization_details():
    payload = MINIMAL | {"auth": {"local_principal": {"authorization_details": [{"type": "payment_initiation", "actions": ["initiate"]}]}}}
    cfg = McpPortalConfig.model_validate(payload)
...
def test_introspection_requires_exactly_one_of_openapi_or_graphql():
    ...
    cfg = IntrospectionConfig.model_validate({"graphql": {"url": "https://api.example.com/graphql"}})
...
def test_graphql_introspection_requires_base_url():
    with pytest.raises(ValidationError, match="base_url"):
        UpstreamConfig.model_validate({"introspection": {"graphql": {"url": "https://api.example.com/graphql"}}})
